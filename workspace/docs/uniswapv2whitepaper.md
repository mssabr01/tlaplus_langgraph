...
v2 core contracts.
It covers the contracts’ new features—including arbitrary pairs
...
price for the two assets over arbitrary intervals. Finally, it enables “ﬂash swaps” where users
...
used to instantiate those contracts.
Actually using Uniswap v2 will require calling the
...
However, this rule imposes signiﬁcant costs on liquidity providers. All liquidity providers
...
A proliferation of pairs between arbitrary ERC-20s could make it somewhat more diﬃcult
to ﬁnd the best path to trade a particular pair, but routing can be handled at a higher layer
...
t
(1)
Since arbitrageurs will trade with Uniswap if this price is incorrect (by a suﬃcient amount
to make up for the fee), the price oﬀered by Uniswap tends to track the relative market price
...
on the inﬂated price), and then sell ETH back to the pair to trade it back to the true price.1
...
an attack, see [3].
2
block). This price is more diﬃcult to manipulate than prices during a block. If the attacker
...
checkpoint the accumulator’s value at t1 and then again at t2, subtract the ﬁrst value from
...
TWAP is less commonly used, and is diﬃcult to compute on Ethereum.
3
...
These numbers have a range of [0, 2112 −1]4 and a precision of
1
## 2112 .
...
will reasonably never overﬂow. In fact, the date when the Unix timestamp overﬂows a uint32
...
across overﬂow intervals can be appropriately accounted for given that oracles are using the
...
out the tokens requested by the user and enforcing the invariant.
Once the callback is
complete, the contract checks the new balances and conﬁrms that the invariant is satisﬁed
4The theoretical upper bound of 2112 −(
1
2112 ) does not apply in this setting, as UQ112.112 numbers in
Uniswap are always generated from the ratio of two uint112s. The largest such ratio is 2112−1
1
= 2112 −1.
4
...
Initially, feeTo is not set, and no fee is collected. A pre-speciﬁed address—feeToSetter—can
...
√k1
√k2 for f1,2 and solving for sm, we
can rewrite this as:
sm =
√k2 −√k1
φ −1) · √k2 + √k1
...
(9)
With ﬂash swaps, Uniswap v2 introduces the possibility that xin and yin might both
...
7
Uniswap v2 handles non-standard implementations diﬀerently. Speciﬁcally, if a transfer()
call9 has no return value, Uniswap v2 interprets it as a success rather than as a failure. This
change should not aﬀect any ERC-20 tokens that conform to the standard (because in those
...
· sstarting
(12)
But what if they are the ﬁrst depositor? In that case, xstarting is 0, so this formula will
not work.
...
was no guarantee that that ratio reﬂected the true price. Additionally, Uniswap v2 supports
...
To mitigate this, Uniswap v2 burns the ﬁrst 1e-15 (0.000000000000001) pool shares that
...
In order to eﬃciently implement the oracle mechanism, Uniswap v2 only support reserve
...
References
[1]
Hayden Adams. 2018. url: zxg/HJ9jLsfTz?
type=view.
[2]
Guillermo Angeris et al. An analysis of Uniswap markets. 2019. arXiv: 1911.03380
[q-fin.TR].
[3]
samczsun. Taking undercollateralized loans for fun and for proﬁt. Sept. 2019. url:
 undercollateralized- loans- for- fun- and-
for-profit/.
[4]
Fabian Vogelsteller and Vitalik Buterin. Nov. 2015. url: 
org/EIPS/eip-20.
[5]
Jordi Baylina Jacques Daﬄon and Thomas Shababi. EIP 777: ERC777 Token Standard.
Nov. 2017. url: .
[6]
Radar. WTF is WETH? url: 
[7]
Uniswap.info. Wrapped Ether (WETH). url: 223fe8d0a0e5c4f27e
[8]
Vitalik Buterin. EIP 1014: Skinny CREATE2. Apr. 2018. url: 
org/EIPS/eip-1014.
4
Disclaimer
...
aﬃliates and does not necessarily reﬂect the opinions of Paradigm, its aﬃliates or individuals
associated with Paradigm. The opinions reﬂected herein are subject to change without being
updated.
10
